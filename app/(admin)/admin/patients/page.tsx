...
export default async function AdminPatientsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#0B1F4E]">Patients</h1>
        <p className="text-slate-500 mt-1 text-sm">View and manage registered patients.</p>
      </div>
      ...
      <Card className="border-border shadow-card">
        ...
        <CardContent>
          {allPatients.length === 0 ? (
            <p className="text-sm text-slate-400 py-4">No patients yet.</p>
          ) : (
                ...
                <tbody className="divide-y divide-border">
                  {allPatients.map((p) => (
                    <tr key={p.id} className="hover:bg-surface transition-colors">
                      ...
                      <td className="py-3 pr-4 font-mono text-xs text-slate-600">{p.mobile}</td>
                      <td className="py-3 pr-4 text-slate-500 text-xs font-mono">{p.pincode ?? "—"}</td>
                      <td className="py-3 pr-4">
