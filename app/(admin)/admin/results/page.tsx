...
const STATUS_MAP: Record<string, { label: string; class: string }> = {
  ...
  released: { label: "Released", class: "bg-[#2DB549]/10 text-[#1D7D31] border-[#2DB549]/20" },
};
...
export default async function AdminResultsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white/95">Results</h1>
        <p className="text-white/60 mt-1 text-sm">Manage sample collection and report release.</p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total Results", value: allResults.length, color: "text-white/95" },
      ...
      <Card className="border-white/20 shadow-xl">
        ...
        <CardContent>
          {allResults.length === 0 ? (
            <p className="text-sm text-white/50 py-4">No results yet. They will appear here once orders are collected.</p>
          ) : (
                  ...
                  {allResults.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="hover:relative z-10 transition-colors">
                        <td className="py-3 pr-4 font-mono text-xs font-semibold text-white/95">
                          #{r.orderNumber ?? "—"}
                        </td>
                        <td className="py-3 pr-4">
                          <p className="font-medium text-white/90">{r.patientName ?? "—"}</p>
                          <p className="text-xs text-white/50 font-mono">{r.patientMobile ?? ""}</p>
                        </td>
                        <td className="py-3 pr-4 text-white/70 text-xs max-w-[150px] truncate">
                          {r.testName ?? "—"}
                        </td>
                        ...
                        <td className="py-3">
                          <div className="flex items-center gap-2">
                            ...
                            {r.status !== "released" && (
                              <button className="text-xs text-white/50 hover:text-[#2DB549] font-medium">
                                Mark Released
                              </button>
                            )}
                          </div>
