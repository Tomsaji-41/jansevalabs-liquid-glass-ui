    ...
    <div className="min-h-screen bg-surface flex">
      <aside className="hidden md:flex w-64 flex-col bg-white border-r border-border">
        <div className="p-4 border-b border-border">
          ...
          <span className="block text-[9px] font-semibold tracking-widest text-slate-400 uppercase mt-1">Admin Panel</span>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map(({ href, icon: Icon, label }) => (
            <Link
              key={href}
              href={href}
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 hover:bg-navy-50 hover:text-[#1E3A8A] transition-colors"
            >
